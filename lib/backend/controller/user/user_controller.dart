...
class UserController {
  static Future<user_model.User?> getUserUid() async {
 ...
 static Future<AuthResult> registerWithEmailAndPassword({
    ...
    AuthResult result = await FirebaseAuthService.registerWithEmailAndPassword(email: email, password: password);

    if (result.ok()){
      UserRepository.addUserData(name: name, age: age, email: email, role: role.toLowerCase(), uid : result.user!.uid);
    }
    return result;
  }
